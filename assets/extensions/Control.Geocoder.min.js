...
a},geocode:function(a,d,e){a={SingleLine:a,outFields:"Addr_Type",forStorage:!1,maxLocations:10,f:"json"};this._key&&this._key.length&&(a.token=this._key);l(this.options.service_url+"/findAddressCandidates",b.extend(a,this.options.geocodingQueryParams),function(a){var c=[];if(a.candidates&&a.candidates.length)for(var h=0;h<=a.candidates.length-1;h++){var k=a.candidates[h],g=b.latLng(k.location.y,k.location.x),m=b.latLngBounds(b.latLng(k.extent.ymax,k.extent.xmax),b.latLng(k.extent.ymin,k.extent.xmin));
...
a;this.options.reverseQueryParams.access_token=a},geocode:function(a,d,e){var c=this.options.geocodingQueryParams;void 0!==c.proximity&&void 0!==c.proximity.lat&&void 0!==c.proximity.lng&&(c.proximity=c.proximity.lng+","+c.proximity.lat);l(this.options.serviceUrl+encodeURIComponent(a)+".json",c,function(a){var c=[];if(a.features&&a.features.length)for(var f=0;f<=a.features.length-1;f++){for(var g=a.features[f],m=b.latLng(g.center.reverse()),l=g.bbox?b.latLngBounds(b.latLng(g.bbox.slice(0,2).reverse()),
b.latLng(g.bbox.slice(2,4).reverse())):b.latLngBounds(m,m),p={text:g.text,address:g.address},n=0;n<(g.context||[]).length;n++){var q=g.context[n].id.split(".")[0];p[q]=g.context[n].text}c[f]={name:g.place_name,bbox:l,center:m,properties:p}}d.call(e,c)})},suggest:function(a,b,e){return this.geocode(a,b,e)},reverse:function(a,d,e,c){l(this.options.serviceUrl+encodeURIComponent(a.lng)+","+encodeURIComponent(a.lat)+".json",this.options.reverseQueryParams,function(a){var d=[];if(a.features&&a.features.length)for(var f=
0;f<=a.features.length-1;f++){var g=a.features[f],m=b.latLng(g.center.reverse()),l=g.bbox?b.latLngBounds(b.latLng(g.bbox.slice(0,2).reverse()),b.latLng(g.bbox.slice(2,4).reverse())):b.latLngBounds(m,m);d[f]={name:g.place_name,bbox:l,center:m}}e.call(c,d)})}}),C=b.Class.extend({options:{serviceUrl:"https://www.mapquestapi.com/geocoding/v1"},initialize:function(a,d){this._key=decodeURIComponent(a);b.Util.setOptions(this,d)},_formatName:function(){var a=[],b;for(b=0;b<arguments.length;b++)arguments[b]&&
a.push(arguments[b]);return a.join(", ")},geocode:function(a,d,e){l(this.options.serviceUrl+"/address",{key:this._key,location:a,limit:5,outFormat:"json"},b.bind(function(a){var c=[];if(a.results&&a.results[0].locations)for(var h=a.results[0].locations.length-1;0<=h;h--){var k=a.results[0].locations[h],g=b.latLng(k.latLng);c[h]={name:this._formatName(k.street,k.adminArea4,k.adminArea3,k.adminArea1),bbox:b.latLngBounds(g,g),center:g}}d.call(e,c)},this))},reverse:function(a,d,e,c){l(this.options.serviceUrl+
"/reverse",{key:this._key,location:a.lat+","+a.lng,outputFormat:"json"},b.bind(function(a){var d=[];if(a.results&&a.results[0].locations)for(var f=a.results[0].locations.length-1;0<=f;f--){var g=a.results[0].locations[f],l=b.latLng(g.latLng);d[f]={name:this._formatName(g.street,g.adminArea4,g.adminArea3,g.adminArea1),bbox:b.latLngBounds(l,l),center:l}}e.call(c,d)},this))}}),D=b.Class.extend({options:{userId:"<insert your userId here>",apiKey:"<insert your apiKey here>",serviceUrl:"https://neutrinoapi.com/"},
initialize:function(a){b.Util.setOptions(this,a)},geocode:function(a,d,e){l(this.options.serviceUrl+"geocode-address",{apiKey:this.options.apiKey,userId:this.options.userId,address:a.split(/\s+/).join(".")},function(a){var c=[];if(a.locations){a.geometry=a.locations[0];var h=b.latLng(a.geometry.latitude,a.geometry.longitude),k=b.latLngBounds(h,h);c[0]={name:a.geometry.address,bbox:k,center:h}}d.call(e,c)})},suggest:function(a,b,e){return this.geocode(a,b,e)},reverse:function(a,d,e,c){l(this.options.serviceUrl+
"geocode-reverse",{apiKey:this.options.apiKey,userId:this.options.userId,latitude:a.lat,longitude:a.lng},function(d){var f=[];if(200==d.status.status&&d.found){var k=b.latLng(a.lat,a.lng),g=b.latLngBounds(k,k);f[0]={name:d.address,bbox:g,center:k}}e.call(c,f)})}}),r=b.Class.extend({options:{serviceUrl:"https://nominatim.openstreetmap.org/",geocodingQueryParams:{},reverseQueryParams:{},htmlTemplate:function(a){a=a.address;var b=[];(a.road||a.building)&&b.push("{building} {road} {house_number}");(a.city||
a.town||a.village||a.hamlet)&&b.push('<span class="'+(0<b.length?"leaflet-control-geocoder-address-detail":"")+'">{postcode} {city} {town} {village} {hamlet}</span>');(a.state||a.country)&&b.push('<span class="'+(0<b.length?"leaflet-control-geocoder-address-context":"")+'">{state} {country}</span>');return R(b.join("<br/>"),a,!0)}},initialize:function(a){b.Util.setOptions(this,a)},geocode:function(a,d,e){l(this.options.serviceUrl+"search",b.extend({q:a,limit:5,format:"json",addressdetails:1},this.options.geocodingQueryParams),
b.bind(function(a){for(var c=[],h=a.length-1;0<=h;h--){for(var k=a[h].boundingbox,g=0;4>g;g++)k[g]=parseFloat(k[g]);c[h]={icon:a[h].icon,name:a[h].display_name,html:this.options.htmlTemplate?this.options.htmlTemplate(a[h]):void 0,bbox:b.latLngBounds([k[0],k[2]],[k[1],k[3]]),center:b.latLng(a[h].lat,a[h].lon),properties:a[h]}}d.call(e,c)},this))},reverse:function(a,d,e,c){l(this.options.serviceUrl+"reverse",b.extend({lat:a.lat,lon:a.lng,zoom:Math.round(Math.log(d/256)/Math.log(2)),addressdetails:1,
format:"json"},this.options.reverseQueryParams),b.bind(function(a){var d=[];if(a&&a.lat&&a.lon){var f=b.latLng(a.lat,a.lon);d.push({name:a.display_name,html:this.options.htmlTemplate?this.options.htmlTemplate(a):void 0,center:f,bounds:b.latLngBounds(f,f),properties:a})}e.call(c,d)},this))}}),E=b.Class.extend({options:{OpenLocationCode:void 0,codeLength:void 0},initialize:function(a){b.setOptions(this,a)},geocode:function(a,d,e){try{var c=this.options.OpenLocationCode.decode(a),f={name:a,center:b.latLng(c.latitudeCenter,
c.longitudeCenter),bbox:b.latLngBounds(b.latLng(c.latitudeLo,c.longitudeLo),b.latLng(c.latitudeHi,c.longitudeHi))};d.call(e,[f])}catch(h){console.warn(h),d.call(e,[])}},reverse:function(a,d,e,c){try{var f={name:this.options.OpenLocationCode.encode(a.lat,a.lng,this.options.codeLength),center:b.latLng(a.lat,a.lng),bbox:b.latLngBounds(b.latLng(a.lat,a.lng),b.latLng(a.lat,a.lng))};e.call(c,[f])}catch(h){console.warn(h),e.call(c,[])}}}),F=b.Class.extend({options:{serviceUrl:"https://api.opencagedata.com/geocode/v1/json"},
initialize:function(a){this._accessToken=a},geocode:function(a,d,e){l(this.options.serviceUrl,{key:this._accessToken,q:a},function(a){var c=[];if(a.results&&a.results.length)for(var h=0;h<a.results.length;h++){var k=a.results[h],g=b.latLng(k.geometry),l=k.annotations&&k.annotations.bounds?b.latLngBounds(b.latLng(k.annotations.bounds.northeast),b.latLng(k.annotations.bounds.southwest)):b.latLngBounds(g,g);c.push({name:k.formatted,bbox:l,center:g})}d.call(e,c)})},suggest:function(a,b,e){return this.geocode(a,
b,e)},reverse:function(a,d,e,c){l(this.options.serviceUrl,{key:this._accessToken,q:[a.lat,a.lng].join()},function(a){var d=[];if(a.results&&a.results.length)for(var f=0;f<a.results.length;f++){var g=a.results[f],l=b.latLng(g.geometry),n=g.annotations&&g.annotations.bounds?b.latLngBounds(b.latLng(g.annotations.bounds.northeast),b.latLng(g.annotations.bounds.southwest)):b.latLngBounds(l,l);d.push({name:g.formatted,bbox:n,center:l})}e.call(c,d)})}}),p=b.Class.extend({options:{serviceUrl:"https://api.geocode.earth/v1",
...
k=c.getCenter();else f.feature.bbox?(k=f.getLatLng(),c=b.latLngBounds(b.GeoJSON.coordsToLatLng(f.feature.bbox.slice(0,2)),b.GeoJSON.coordsToLatLng(f.feature.bbox.slice(2,4)))):(k=f.getLatLng(),c=b.latLngBounds(k,k));a.name=f.feature.properties.label;a.center=k;a[d]=c;a.properties=f.feature.properties;e.push(a)}});return e}}),t=p,G=p,H=G.extend({options:{serviceUrl:"https://api.openrouteservice.org/geocode"}}),I=b.Class.extend({options:{serviceUrl:"https://photon.komoot.de/api/",reverseUrl:"https://photon.komoot.de/reverse/",
